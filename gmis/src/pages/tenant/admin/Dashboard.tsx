// ============================================================
// GMIS — Admin Dashboard
// FIXED:
//   - loadSettings() uses .maybeSingle() — no longer crashes for new schools
//   - initialTab prop properly syncs to tab state on route change
//   - loadAll() errors are caught individually so one failure doesn't blank everything
//   - Tab state derived from URL path when no initialTab prop provided
// ============================================================
...
// ── TYPES ─────────────────────────────────────────────────
type Tab =
  | 'dashboard' | 'approvals' | 'students' | 'courses'
  | 'results'   | 'timetable' | 'fees'     | 'idcards'
  | 'elections' | 'news'      | 'paystack' | 'settings'
  | 'academic'
...
interface Lecturer    { id: string; full_name: string; email: string }
interface Department  { id: string; name: string; code: string }
interface FeeType     { id: string; name: string; description: string }
interface FeeItem     { id: string; amount: number; session: string; is_active: boolean; fee_types: { name: string } }
interface NewsItem    { id: string; title: string; content: string; is_published: boolean; author_name: string; created_at: string }
...
export default function AdminDashboard({ initialTab }: { initialTab?: Tab }) {
  ...
  }, [location.pathname, initialTab, tabFromPath])
  ...
  // ── LOAD ALL — individual catches so one failing query doesn't kill everything ──
  const loadAll = async () => {
    setLoading(true)
  ...
  const loadCourses = async () => {
    ...
    const { data } = await db.from('courses')
      .select('*, departments(name), lecturers(full_name)')
      .order('course_code')
    if (data) {
  ...
  const loadLecturers = async () => {
    if (!db) return
    const { data } = await db.from('lecturers').select('id,full_name,email').eq('is_active', true)
    if (data) setLecturers(data)
  }

  const loadDepartments = async () => {
    if (!db) return
    const { data } = await db.from('departments').select('id,name,code').eq('is_active', true).order('name')
  ...
  // FIXED: Use maybeSingle() — single() throws 406 when no row exists (new schools)
  const loadSettings = async () => {
    if (!db) return
    const { data, error } = await db.from('org_settings').select('*').maybeSingle()
    if (error) {
      console.warn('org_settings not found — school may be newly approved:', error.message)
      return
    }
    if (data) {
      setOrgSettings(data)
      setPaystackKey(data.paystack_public_key || '')
      setPaystackSec(data.paystack_secret_key || '')
    }
  }
  ...
  // Add student — FIXED order: insert DB record first, then create auth account
  const addStudent = async () => {
    if (!db) return
    if (!studForm.first_name || !studForm.last_name || !studForm.matric_number || !studForm.email) {
      toast.error('Name, matric number and email are required')
      return
    }
    setSavingStud(true)
    try {
      // 1. Insert student record first (no supabase_uid yet)
      const { data: newStudent, error: insertError } = await db.from('students').insert({
      ...
      // 2. Create auth account (if it fails, student record still exists — admin can fix)
      const { data: authData, error: authError } = await db.auth.signUp({
        ...
        options: {
          data: {
            role:          'student',
            full_name:     `${studForm.first_name.trim()} ${studForm.last_name.trim()}`,
            matric_number: studForm.matric_number.trim().toUpperCase(),
          },
        },
      })

      // 3. Link auth UID to student record if auth succeeded
      if (authData?.user?.id && newStudent?.id) {
        await db.from('students').update({ supabase_uid: authData.user.id }).eq('id', newStudent.id)
      }

      if (authError) {
        toast.success(`${studForm.first_name} added! Note: email account setup failed — student can use "Forgot Password" to set their password.`)
      } else {
  ...
  const deleteCourse = async (id: string, code: string) => {
    if (!db) return
    if (!confirm(`Delete course ${code}? This will also remove all results for this course.`)) return
  ...
  const releaseResults = async (courseCode: string) => {
    ...
    toast.success(`Results released for ${courseCode}!`)
    loadResults()
  }

  const unlockResults = async (courseCode: string) => {
    if (!db) return
    const ids = results.filter(r => r.courses?.course_code === courseCode).map(r => r.id)
    await db.from('results').update({ is_locked: false, submitted_at: null }).in('id', ids)
    toast.success(`Results unlocked for ${courseCode}`)
    loadResults()
  }
  ...
  const addFee = async () => {
    ...
    setFeeForm({ fee_type_id: '', amount: '', session: '2024/2025' })
  ...
  const savePaystack = async () => {
    ...
    const { error } = await db.from('org_settings').upsert({
      ...
      paystack_secret_key: paystackSec,
    })
    setSavePay(false)
    if (error) toast.error('Failed to save')
    else toast.success('Paystack config saved!')
  }

  // ── HELPERS ────────────────────────────────────────────
  const filteredStudents = students.filter(s => {
    const matchSearch = !search ||
      `${s.first_name} ${s.last_name} ${s.matric_number} ${s.email}`
  ...
  const sc: Record<string, string> = {
    active: '#4ade80', pending: '#fbbf24', suspended: '#f87171', graduated: '#60a5fa',
  }

  const navToTab = (t: Tab) => {
    const pathMap: Record<Tab, string> = {
      dashboard: '/admin', approvals: '/admin/approvals', students: '/admin/students',
      courses: '/admin/courses', results: '/admin/results', fees: '/admin/fees',
      news: '/admin/news', paystack: '/admin/paystack', settings: '/admin/settings',
      academic: '/admin/academic', timetable: '/admin/timetable',
      idcards: '/admin/idcards', elections: '/admin/elections',
    }
    navigate(pathMap[t])
  }
  ...
  if (loading) return (
    <SidebarLayout active="dashboard" role="admin">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh', flexDirection: 'column', gap: 14 }}>
        <div style={S.spin} />
        <p style={{ color: '#7a8bbf', fontSize: 14 }}>Loading admin panel...</p>
      </div>
    </SidebarLayout>
  )
  ...
  return (
    <SidebarLayout active={tab} role="admin">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20, flexWrap: 'wrap', gap: 10 }}>
        ...
        {pending.length > 0 && (
          <button
            onClick={() => navToTab('approvals')}
            style={{ padding: '8px 16px', background: 'rgba(251,191,36,.12)', border: '1px solid rgba(251,191,36,.3)', borderRadius: 10, color: '#fbbf24', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}
          >
        ...
        {([
          ...
          >
            ...
            {id === 'approvals' && pending.length > 0 && (
              <span style={{ marginLeft: 5, background: '#f87171', color: '#fff', fontSize: 10, padding: '1px 6px', borderRadius: 100 }}>
                {pending.length}
              </span>
            )}
          </button>
        ...
        <>
          ...
          <div style={S.card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
              ...
              <button onClick={() => navToTab('students')} style={S.btnSm}>View all →</button>
            </div>
            ...
            <div style={S.formCard}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: 12 }}>
                {([
                  ...
                  ['matric_number',  'Matric number *',  'text',  'e.g. 02SCSC026'],
                  ['email',         'Email address *',  'email', 'student@email.com'],
                  ['phone',         'Phone',            'tel',   '+234 800 000 0000'],
                ] as [keyof typeof studForm, string, string, string][]).map(([field, label, type, ph]) => (
                  <div key={field}>
                    <label style={S.label}>{label}</label>
                    <input
                      style={S.input} type={type} placeholder={ph}
                      value={studForm[field] as string}
                      onChange={e => setStudForm(p => ({ ...p, [field]: e.target.value }))}
                    />
                  </div>
                ))}
                <div>
      ...
      {tab === 'courses' && (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, flexWrap: 'wrap', gap: 10 }}>
            <div>
              <h2 style={S.ct}>Course management</h2>
              <p style={{ fontSize: 13, color: '#7a8bbf' }}>Assign lecturers directly from the dropdown</p>
            </div>
                    ...
                    <tr key={c.id}>
                      ...
                      <td style={S.td}>
                        <select
                          ...
                          style={{ ...S.input, width: 160, padding: '5px 8px', fontSize: 11 }}
                        >
        ...
        <>
          ...
          <div style={S.card}>
            {courseCodes.length === 0
              ? <Empty icon="📊" text="No results uploaded by lecturers yet." />
            ...
            <div style={S.formCard}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                <div>
                  ...
                  <select style={S.input} value={feeForm.fee_type_id} onChange={e => setFeeForm(p => ({ ...p, fee_type_id: e.target.value }))}>
                ...
                <div>
                  ...
                  <input style={S.input} type="number" value={feeForm.amount} onChange={e => setFeeForm(p => ({ ...p, amount: e.target.value }))} placeholder="e.g. 150000" />
                </div>
                <div>
                  <label style={S.label}>Session</label>
                  <input style={S.input} value={feeForm.session} onChange={e => setFeeForm(p => ({ ...p, session: e.target.value }))} />
                </div>
              </div>
              <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
      ...
      {tab === 'news' && (
        <>
          ...
          {showNewsForm && (
            <div style={S.formCard}>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <textarea style={{ ...S.input, minHeight: 100, resize: 'vertical' } as React.CSSProperties} value={newsForm.content} onChange={e => setNewsForm(p => ({ ...p, content: e.target.value }))} placeholder="Write your announcement here..." />
              </div>
          ...
          <div style={S.card}>
            {news.length === 0
              ? <Empty icon="📰" text="No news posts yet." />
              : news.map(n => (
      ...
      {tab === 'paystack' && (
        <>
          ...
          <div style={{ padding: '13px 16px', background: 'rgba(96,165,250,0.08)', border: '1px solid rgba(96,165,250,0.2)', borderRadius: 12, marginBottom: 20, fontSize: 13, color: '#60a5fa' }}>
            ℹ Link your school's Paystack API keys. All student payments go 100% to your account.
          </div>
          <div style={S.card}>
            <div style={{ marginBottom: 14 }}>
              <label style={S.label}>Paystack public key</label>
              <input style={S.input} value={paystackKey} onChange={e => setPaystackKey(e.target.value)} placeholder="pk_live_xxxxxxxxxx" />
            </div>
            <div style={{ marginBottom: 20 }}>
              <label style={S.label}>Paystack secret key</label>
              <input style={S.input} type="password" value={paystackSec} onChange={e => setPaystackSec(e.target.value)} placeholder="sk_live_xxxxxxxxxx" />
            </div>
      ...
      {tab === 'settings' && (
        <div style={S.card}>
            ...
            {([
              ...
              <div key={field}>
                ...
                <input
                  ...
                  onChange={e => setOrgSettings(p => ({ ...p, [field]: e.target.value }))}
                  placeholder={label}
                />
              </div>
            ))}
          ...
          <button
            onClick={async () => {
              if (!db) return
              const { error } = await db.from('org_settings').upsert(orgSettings)
...
function StudentTable({
  students, onApprove, onSuspend, sc, showAll,
}: {
  students: Student[]
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 520 }}>
        <thead>
          <tr>
            {(['Student', 'Matric no.', 'Level', showAll && 'Dept.', showAll && 'Status', showAll && 'Registered', 'Actions'] as (string | false)[])
              .filter(Boolean)
              .map(h => <th key={h as string} style={S.th}>{h as string}</th>)}
          </tr>
        </thead>
        <tbody>
          {students.map(s => {
              ...
              <tr key={s.id}>
                <td style={S.td}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div>
                      <div style={{ fontWeight: 600, color: '#e8eeff', fontSize: 13 }}>{name}</div>
                      <div style={{ fontSize: 11, color: '#7a8bbf' }}>{s.email}</div>
                    </div>
                  </div>
                </td>
                ...
                {showAll && (
                  <td style={S.td}>
                    <span style={{ fontSize: 11, fontWeight: 700, background: (sc[s.status] || '#7a8bbf') + '20', color: sc[s.status] || '#7a8bbf', padding: '2px 9px', borderRadius: 100, textTransform: 'capitalize' }}>
                      {s.status}
                    </span>
                  </td>
                )}
                ...
                <td style={S.td}>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {s.status === 'pending' && (
                      <button onClick={() => onApprove(s.id, name)} style={{ ...S.btnSm, background: 'rgba(74,222,128,.12)', color: '#4ade80', border: '1px solid rgba(74,222,128,.25)', fontWeight: 700 }}>✓ Approve</button>
                    )}
                    {s.status === 'active' && (
                      <button onClick={() => onSuspend(s.id, name)} style={{ ...S.btnSm, background: 'rgba(248,113,113,.1)', color: '#f87171', border: '1px solid rgba(248,113,113,.2)' }}>Suspend</button>
                    )}
                    {s.status === 'suspended' && (
                      <button onClick={() => onApprove(s.id, name)} style={{ ...S.btnSm, background: 'rgba(74,222,128,.12)', color: '#4ade80', border: '1px solid rgba(74,222,128,.25)' }}>Reactivate</button>
                    )}
