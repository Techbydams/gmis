...
// ★ Student name loaded from Supabase DB — fully personalized
// ============================================================
...
export default function StudentDashboard() {
  ...
  const [stats,    setStats]    = useState({ courses: 0, paidFees: 0, totalFees: 0 })
  ...
  const load = async () => {
    setLoading(true)
    try {
      // 1 — Load student profile first (need student.id for everything else)
      ...
      // 2 — Load rest in parallel using student.id
      const sid = s.id
      await Promise.all([loadClasses(sid), loadNotifs(sid), loadStats(sid)])
    } finally {
      setLoading(false)
    }
  }
  ...
  const loadClasses = async (sid: string) => {
    ...
    const { data } = await db!
      .from('timetable').select('*, courses(course_code, course_name)')
      .in('course_id', regs.map((r: any) => r.course_id))
      .eq('day_of_week', today).order('start_time')
  ...
  const loadStats = async (sid: string) => {
    const [{ count: c1 }, { count: c2 }, { count: c3 }] = await Promise.all([
      db!.from('semester_registrations').select('*',{count:'exact',head:true}).eq('student_id',sid).eq('status','registered'),
      db!.from('student_payments').select('*',{count:'exact',head:true}).eq('student_id',sid).eq('status','success'),
      db!.from('fee_structure').select('*',{count:'exact',head:true}).eq('is_active',true),
    ])
    setStats({ courses: c1||0, paidFees: c2||0, totalFees: c3||0 })
  }
  ...
  if (loading) return (
    <SidebarLayout active="dashboard">
      <div style={{display:'flex',alignItems:'center',justifyContent:'center',height:'60vh',flexDirection:'column',gap:16}}>
        <div style={S.spin}/><p style={{color:'#7a8bbf',fontSize:14}}>Loading your dashboard...</p>
      </div>
    </SidebarLayout>
  )
  ...
  if (student?.status === 'pending') return (
    <div style={{minHeight:'100vh',background:'#03071a',display:'flex',alignItems:'center',justifyContent:'center',padding:24,fontFamily:"'DM Sans',system-ui",textAlign:'center'}}>
      <div style={{maxWidth:440}}>
        ...
        {/* ★ Name from DB even on pending screen ★ */}
        <p style={{color:'#7a8bbf',lineHeight:1.8,marginBottom:24}}>
          Hi <strong style={{color:'#e8eeff'}}>{student.first_name}</strong>, your registration is pending admin approval at <strong style={{color:'#e8eeff'}}>{tenant?.name}</strong>. You'll be emailed at <strong style={{color:'#60a5fa'}}>{user?.email}</strong> once activated.
        </p>
        <button onClick={async()=>{await signOut();navigate('/login')}} style={{padding:'10px 24px',background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:11,color:'#7a8bbf',cursor:'pointer',fontSize:13}}>Sign out</button>
      </div>
    </div>
  )

  // ★ NAME COMES FROM DATABASE — never hardcoded ★
  ...
  return (
    <SidebarLayout active="dashboard">
      ...
      {/* ── HEADER with personalized name from DB ── */}
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:22,flexWrap:'wrap',gap:12}}>
        <div>
          <h1 style={{fontFamily:"'Syne',system-ui",fontWeight:800,fontSize:'clamp(18px,3vw,24px)',color:'#e8eeff',marginBottom:4}}>
            {greeting()},{' '}
            {/* ★ This name is pulled live from the students table in Supabase ★ */}
        ...
        <div style={{display:'flex',gap:10,alignItems:'center'}}>
          ...
          {/* Avatar also uses initials from DB */}
          <div
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(150px,1fr))',gap:13,marginBottom:20}}>
        {[
          ...
          {label:'Fees paid',   value:`${stats.paidFees}/${stats.totalFees}`, sub:'Items', icon:'💳', color:stats.paidFees===stats.totalFees&&stats.totalFees>0?'#4ade80':'#fbbf24'},
          {label:'Attendance',  value:'87%',                       sub:'This semester', icon:'✅', color:'#4ade80'},
      ...
      <div style={S.card}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
          <h3 style={{...S.ct,margin:0}}>
            Notifications
            {unread>0&&<span style={{marginLeft:8,background:'#f87171',color:'#fff',fontSize:10,fontWeight:700,padding:'2px 7px',borderRadius:100}}>{unread} new</span>}
          </h3>
          {unread>0&&<button onClick={markRead} style={{fontSize:12,color:'#60a5fa',background:'none',border:'none',cursor:'pointer'}}>Mark all read</button>}
        </div>
        {notifs.length===0
          ?<div style={{textAlign:'center',padding:'18px 0',color:'#3d4f7a',fontSize:13}}>No notifications yet, {firstName}.</div>
          :notifs.map(n=>(
            <div key={n.id} style={{display:'flex',gap:12,alignItems:'flex-start',padding:'10px 0',borderBottom:'1px solid rgba(255,255,255,0.05)',opacity:n.is_read?0.55:1,transition:'opacity .2s'}}>
...
const S: Record<string,React.CSSProperties> = {
  ...
  stat: {background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:16,padding:'16px 18px',transition:'transform .2s'},
